...
- On both platforms, confirm Sideleaf is offered for `.md` and `.txt` files, make it the
...
Launch Services then reads the new bundle identity and Markdown/text document types. On Windows the identifier changes
...
per-user Markdown/text capabilities without changing the user's selected defaults.
