...
try {
  ...
  $window = if ($request.window) { @($windows | Where-Object title -eq $request.window) } else { @($windows | Where-Object title -like '*Sideleaf*') }
  ...
  $result.windows = @($windows | ForEach-Object { @{title=$_.title;processId=$_.processId} })
